...
python scripts/add_experience.py "Fixed memory leak in async handler" \
  --category debugging \
  --tags "python,async,memory" \
  --content "The issue was caused by..."

# Add a new principle
python scripts/add_principle.py "Never block the UI thread" \
  --category architecture \
  --priority high \
  --content "Always offload heavy work..."

# Search memories
python scripts/search_memory.py "async"
...
```bash
python scripts/add_experience.py <title> [options]
...
**Example:**
```bash
python scripts/add_experience.py "Async database query timeout" \
  --category debugging \
...
```bash
python scripts/add_principle.py <title> [options]
...
**Example:**
```bash
python scripts/add_principle.py "Validate inputs at system boundaries" \
  --category security \
  --priority high \
...
```bash
python scripts/search_memory.py <query> [options]
...
python scripts/search_memory.py timeout --type experience --category debugging
```
...
```bash
python scripts/list_memories.py [options]
...
```bash
# List all principles
python scripts/list_memories.py --type principle

# List recent experiences
python scripts/list_memories.py --type experience --sort date
```
...
### references/
- `experience-template.md` - Template for experiences
- `principle-template.md` - Template for principles
- `file-organization.md` - Detailed file structure guide

### assets/templates/
- `experience_template.md` - Default experience template
- `principle_template.md` - Default principle template
...
python scripts/add_experience.py "Race condition in cache invalidation" \
  --category debugging \
...
python scripts/add_principle.py "Use atomic operations for cache invalidation" \
  --category architecture \
  --priority high \
  --content "## Definition
...
python scripts/search_memory.py "cache race"
```
