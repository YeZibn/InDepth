...
推荐频率（经验值）：
- 复杂任务（≥4 子任务）：SubAgent 覆盖率 **SHOULD ≥ 40%**
- 检索/分析型任务：可并行分支 **SHOULD 尽量全部 SubAgent 化**
- 主 Agent 保持“调度 + 汇总”角色，直接执行动作 **SHOULD 控制在关键收敛步骤**

### 4.3 不创建 SubAgent 的例外条件
