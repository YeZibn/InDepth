...
4. Runtime 基于失败事实调用单次 `LLM recovery assessment`
5. 将失败解释、`retry_guidance` 与恢复决策回写到 todo
6. 再单独推导 subtask 当前应进入的状态
7. 然后由 `plan_task_recovery` 在 guardrails 内判断能否原地恢复
8. 只有原地恢复不合适时，才派生 recovery subtask
9. 恢复完成后，可通过 `reopen_subtask` 显式回到原 subtask 主线
10. task 完成后，当前 todo 绑定被关闭
...
- 单次 `LLM recovery assessment`：输出 `failure_interpretation + recovery_decision + retry_guidance`
- `update_task_status`：单独维护 subtask 生命周期状态
- `plan_task_recovery`：基于解释后的 fallback 与 guardrails 产出稳定恢复决策
...
## 2.2 单次 LLM Recovery Assessment 详解

前面的 2.1 讲的是失败恢复主线。这里单独讲当前设计里已经落地的单次 `LLM recovery assessment`，避免把它混在总流程节点里看不清。

这部分最重要的结论只有一句：
- 它是失败出口上的一次独立 LLM 调用
- 它同时负责失败解释与恢复建议
- `plan_task_recovery` 则退化为 guardrails 与稳定落地层
...
3. 调用单次 `LLM recovery assessment`
4. 系统把 LLM 输出按规则边界做归一化与裁剪
5. 将 `failure_interpretation / retry_guidance` 回写到 `fallback_record`
6. `update_task_status(...)` 推导并写入 subtask 生命周期状态
7. `plan_task_recovery(...)` 读取解释后的 fallback 并输出稳定恢复决策
8. 再决定是否 `append_followup_subtasks(...)`

也就是说，这个 assessment 并不负责“发现失败”，它消费的是已经整理好的失败上下文。
...
当前实现里，这次 LLM assessment 接收的核心输入可以分成四组：
...
当前实现里，单次 LLM assessment 是默认主路径，但仍然保留保守 fallback。
...
系统都会回退到保守 fallback interpretation + rule decision。
...
单次 `LLM recovery assessment` 是失败解释与恢复动作的主导层，`plan_task_recovery` 是建立在它之上的稳定 guardrail 层。
...
5. 是否允许自动派生 recovery subtask
   只有在恢复决策明确需要派生，且自动推进条件满足时，Runtime 才会调用 `append_followup_subtasks`。

#### 7.7.2 LLM 辅助的部分

LLM 在失败处理里负责“解释 + 建议”的主判断。

当前更适合交给 LLM 辅助的部分包括：

1. `reason_code / reason_detail` 的上下文化判断
   把失败事实解释成更贴近任务语义的失败类型，例如：
   - 把普通 `HTTP 504` 区分成瞬时后端波动，还是大生成负载过高
   - 把工具失败区分成环境问题、brief 过大，还是执行 owner 不合适

2. `primary_action / recommended_actions`
   也就是下一步更适合：
   - 原地修复
   - 缩小范围
   - handoff
...
3. `retry_guidance`
   把下一轮 prompt 应该怎么缩范围、降复杂度、改变执行方式结构化输出。

4. `reason_detail` 的自然语言补充
   把失败事实解释得更可读，例如：
   - 哪个命令失败了
   - 缺少了什么信息
   - 哪个验证点没过
...
6. 模糊场景下的候选 follow-up subtasks
   当恢复动作已经独立成新工作单元时，LLM 可以给出更像样的 `next_subtasks` 草案；最终是否自动追加仍由 Runtime 决定。

#### 7.7.3 一句话总结

在当前实现里：
- LLM 负责定性与给建议：失败属于什么、下一步怎么恢复
- 规则负责收边：状态怎么变、哪些动作不能越界、何时必须停止自动恢复
...
当前 `plan_task_recovery` 不会把所有失败都压成统一的“retry”。
...
4. 基于 `failure_state/reason_code` 推导 subtask 状态，并单独调用 `update_task_status`
5. `plan_task_recovery`
6. 仅当 `needs_derived_recovery_subtask=true` 且恢复决策允许自动推进时，才 `append_followup_subtasks`
