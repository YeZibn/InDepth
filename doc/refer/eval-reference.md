...
│  │                   EvalOrchestrator                                │   │
│  │                                                                  │   │
│  │  evaluate(task_spec, run_outcome) ──▶ RunJudgement              │   │
│  │         │                                                       │   │
│  │         ▼                                                       │   │
│  │  ┌─────────────────────────────────────────────────────────┐   │   │
│  │  │              Verifier 链路 (有序执行)                       │   │   │
...
│      if avg_score < task_spec.soft_score_threshold:           │
...
class TaskSpec:
    ...
    expected_artifacts: List[str]            # 期望产出物
    soft_score_threshold: float = 0.7        # 软检查阈值
    llm_judge_enabled: bool = False         # 是否启用 LLM 评判
    llm_judge_rubric: Optional[str] = None  # LLM 评判标准
```
...
class LLMJudgeVerifier(BaseVerifier):
    ...
    def verify(self, task_spec, run_outcome) -> VerifierResult:
        # 1. 构建 VerifierAgent
        # 2. 执行 LLM 评判
