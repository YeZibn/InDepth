...
这会导致一个关键问题：
1. 即使触发了 `strong` 压缩，裁剪后上下文仍可能超过 token 安全阈值。
2. 在“最近 8 轮本身很重”时，压缩会出现“反复触发但几乎无效”的现象。
3. `strong/light` 在当前实现里主要体现在 metadata，实际裁剪深度差异不明显。
...
2. 在 `strong` 模式下提供更激进的预算目标，显著降低 length 风险。
...
3. `target_keep_ratio_strong`：strong 压缩后目标保留比例（新增，默认 0.35）
4. `target_keep_ratio_finalize`：finalize 压缩目标比例（新增，默认 0.50）
5. `min_keep_messages`：无论如何至少保留的消息数（新增，默认 6）

预算计算：
1. `light`: `target_keep_tokens = context_window_tokens * target_keep_ratio_light`
2. `strong`: `target_keep_tokens = context_window_tokens * target_keep_ratio_strong`
...
2. `COMPACTION_TARGET_KEEP_RATIO_STRONG`（默认 0.35）
3. `COMPACTION_TARGET_KEEP_RATIO_FINALIZE`（默认 0.50）
4. `COMPACTION_MIN_KEEP_MESSAGES`（默认 6）

### 6.2 配置兼容

1. 保留 `COMPACTION_KEEP_RECENT_TURNS`，但降级为兜底策略（仅在 token 预算异常或估算不可用时启用）。
2. 保留现有 `round_interval/light_token_ratio/strong_token_ratio` 等触发参数。
