...
5. `min_keep_turns`：无论如何至少保留的轮次数（当前实现默认 3）
...
输入：`rows`, `target_keep_tokens`, `min_keep_turns`
...
4. 保证尾部至少有 `min_keep_turns` 轮（按轮次兜底）。
...
4. `COMPACTION_MIN_KEEP_TURNS`（默认 3）
...
4. `_compact_impl(..., token_budget=None, min_keep_turns=...)`
...
2. 过度压缩导致近期语境损失。
- 缓解：`min_keep_turns` + 一致性守护 + 工具配对保护。
