...
class RuntimeAgentCliCommandTests(unittest.TestCase):
    ...
    def test_mode_task_returns_single_mode_hint(self):
        agent = _FakeAgent()
        mode, output, handled = handle_cli_command(agent, "/mode task", "chat")
        self.assertTrue(handled)
        self.assertEqual(mode, "task")
        self.assertEqual(agent.start_calls, [])
        self.assertIn("仅支持 task 单模式", output)

    def test_mode_chat_returns_removed_hint(self):
        agent = _FakeAgent()
        mode, output, handled = handle_cli_command(agent, "/mode chat", "task")
        self.assertTrue(handled)
        self.assertEqual(mode, "task")
        self.assertEqual(agent.start_calls, [])
        self.assertIn("chat 模式已移除", output)

    def test_task_command_rotates_task_even_from_legacy_mode_value(self):
        agent = _FakeAgent()
        mode, output, handled = handle_cli_command(agent, "/task notion", "chat")
        self.assertTrue(handled)
        self.assertEqual(mode, "task")
        self.assertEqual(agent.start_calls, ["notion"])
