...
class TodoMarkdownRationaleTests(unittest.TestCase):
    def test_create_task_writes_explicit_split_rationale(self):
    ...
    def test_create_task_writes_default_split_rationale_when_missing(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            with (
                patch("app.tool.todo_tool.todo_tool._get_todo_dir", return_value=tmpdir),
                patch("app.tool.todo_tool.todo_tool._emit_obs"),
    ...
    def test_create_task_rejects_empty_split_reason(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            with (
                patch("app.tool.todo_tool.todo_tool._get_todo_dir", return_value=tmpdir),
                patch("app.tool.todo_tool.todo_tool._emit_obs"),
